    ...
    const hier::IntVector& ghost_width_to_fill)
{
    ...
    TBOX_ASSERT(d_special_boundary_conditions_db->keyExists("species_gamma"));
    TBOX_ASSERT(d_special_boundary_conditions_db->keyExists("delta"));
    TBOX_ASSERT(d_special_boundary_conditions_db->keyExists("eta_0"));
    ...
    std::vector<double> gamma_vector = d_special_boundary_conditions_db->getDoubleVector("species_gamma"); // specific heat ratio 
    ...
    const double delta = d_special_boundary_conditions_db->getDouble("delta");
    // Read characteristic length of interface from input
    const double eta_0 = d_special_boundary_conditions_db->getDouble("eta_0");
                    ...
                    for (int k = fill_box_lo_idx[2]; k <= fill_box_hi_idx[2]; k++)
                    {
                        for (int j = fill_box_lo_idx[1]; j <= fill_box_hi_idx[1]; j++)
                        {
                            for (int i = fill_box_lo_idx[0]; i <= fill_box_hi_idx[0]; i++)
                            {
                                ...
                                double X_2_H = 0.5*(1.0 + erf((x[0] - eta_0 - shift)/delta)); // mass fraction of second species (Y_2)
