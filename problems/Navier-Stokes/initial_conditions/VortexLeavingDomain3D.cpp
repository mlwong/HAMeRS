    ...
    else if (d_flow_model_type == FLOW_MODEL::FOUR_EQN_CONSERVATIVE)
    {
        ...
        if (initial_time)
        {
            ...
            else if (d_project_name == "3D vortex leaving domain in z-direction")
            {
                ...
                const double p_inf = double(1)/gamma_mixture;
                const double w_inf = Ma*sqrt(gamma_mixture*p_inf/rho_inf);
                ...
                for (int k = 0; k < patch_dims[2]; k++)
                {
                    for (int j = 0; j < patch_dims[1]; j++)
                    {
                        for (int i = 0; i < patch_dims[0]; i++)
                        {
                            ...
                            rho_v[idx_cell]   = double(0);
                            rho_w[idx_cell]   = rho_vortex*w_vortex;
                            E[idx_cell]       = p_vortex/(gamma_mixture - double(1)) + double(1)/double(2)*rho_vortex*(u_vortex*u_vortex + w_vortex*w_vortex);
                        }
                    }
                }
            }
