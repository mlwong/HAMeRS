    ...
    const bool initial_time)
{
    ...
    if (initial_time)
    {
        ...
        /*
         * Initialize data for a 3D uniform flow.
         */
        ...
        for (int k = -num_ghosts_cons_var[2]; k < patch_dims[2] + num_ghosts_cons_var[2]; k++)
        {
            for (int j = -num_ghosts_cons_var[1]; j < patch_dims[1] + num_ghosts_cons_var[1]; j++)
            {
                for (int i = -num_ghosts_cons_var[0]; i < patch_dims[0] + num_ghosts_cons_var[0]; i++)
                {
                    ...
                    Real x[3];
                    x[0] = Real(patch_xlo[0]) + (Real(i) + half)*Real(dx[0]); // x-coordinate of the point.
                    x[1] = Real(patch_xlo[1]) + (Real(j) + half)*Real(dx[1]); // y-coordinate of the point.
                    x[2] = Real(patch_xlo[2]) + (Real(k) + half)*Real(dx[2]); // z-coordinate of the point.
                    ...
                    // Check this, no w velocity for cylinder, only u and v?
                    w_ic = Real(0);
