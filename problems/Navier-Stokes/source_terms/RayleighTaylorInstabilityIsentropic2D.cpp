    ...
    const int RK_step_number)
{
    ...
    TBOX_ASSERT(d_source_terms_db->keyExists("eta_const"));
    const double eta_0  = d_source_terms_db->getDouble("eta_const");
    ...
    if (d_project_name == "2D smooth Isentropic Rayleigh-Taylor instability")
    {
        TBOX_ASSERT(d_source_terms_db->keyExists("delta_const"));
        const double delta = d_source_terms_db->getDouble("delta_const"); // characteristic length of interface.
        ...
        for (int j = 0; j < patch_dims_1; j++)
        {
            ...
            for (int i = 0; i < patch_dims_0; i++)
            {
                ...
                if (!(x[0] >= d_special_source_box_lo[0] && x[0] <= d_special_source_box_hi[0]))
                {
                    ...
                    if(x[0] < 0.0)
                    {
                        const double erf_start_lo  = double(0.5)*(domain_xlo[0]-d_special_source_box_lo[0]) + d_special_source_box_lo[0]; //center of erf is 0.5 of the way into sponge
                    ...
                    {
                        const double erf_start_hi = double(0.5)*(domain_xhi[0]-d_special_source_box_hi[0]) + d_special_source_box_hi[0]; //center of erf is 0.5 of the way into sponge
