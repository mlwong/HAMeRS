    ...
    const int RK_step_number)
{
    ...
    TBOX_ASSERT(d_source_terms_db->keyExists("species_gamma"));
    TBOX_ASSERT(d_source_terms_db->keyExists("delta"));
    TBOX_ASSERT(d_source_terms_db->keyExists("eta_0"));

    std::vector<double> gamma_vector = d_source_terms_db->getDoubleVector("species_gamma"); // specific heat ratio 
    const double gamma = gamma_vector[0]; // specific heat ratio for heavier gas (assume both gases have same specific heat ratios)
    const double delta = d_source_terms_db->getDouble("delta"); // Read characteristic thickness of interface from input
    const double eta_0 = d_source_terms_db->getDouble("eta_0"); // Read characteristic length of interface from input
        ...
        for (int k = 0; k < patch_dims_2; k++)
        {
            for (int j = 0; j < patch_dims_1; j++)
            {
                ...
                for (int i = 0; i < patch_dims_0; i++)
                {
                    ...
                    const double X_2_H  = 0.5*(1.0 + erf((x[0] - eta_0)/delta)); // mass fraction of second species (Y_2)
                    ...
                    if (!(x[0] >= d_special_source_box_lo[0] && x[0] <= d_special_source_box_hi[0]))
                    {
                        ...
                        if(x[0] < 0.0)
                        {
                            const double erf_start_lo  = double(0.5)*(domain_xlo[0]-d_special_source_box_lo[0]) + d_special_source_box_lo[0]; //center of erf is 3/4 of the way into sponge
                            const double erf_offset_lo = double(-0.5) * erf((d_special_source_box_lo[0]-erf_start_lo)/(std::abs(erf_start_lo)*double(0.1))) + double(0.5); //value of erf at start of sponge
                            xi_b        = double(-0.5) * erf((x[0]-erf_start_lo)/(std::abs(erf_start_lo)*double(0.1))) + double(0.5) - erf_offset_lo; //subtract value of erf at start of sponge to start xi_b at zero
                            // xi_b        = 1.0;
                        }
                        else
                        {
                            const double erf_start_hi = double(0.5)*(domain_xhi[0]-d_special_source_box_hi[0]) + d_special_source_box_hi[0]; //center of erf is 3/4 of the way into sponge
                            const double erf_offset_hi = double(0.5) * erf((d_special_source_box_hi[0]-erf_start_hi)/(std::abs(erf_start_hi)*double(0.1))) + double(0.5); //value of erf at start of sponge
                            xi_b            = double(0.5) * erf((x[0]-erf_start_hi)/(std::abs(erf_start_hi)*double(0.1))) + double(0.5) - erf_offset_hi; //subtract value of erf at start of sponge to start xi_b at zero
                            // xi_b = 1.0;
                        }
