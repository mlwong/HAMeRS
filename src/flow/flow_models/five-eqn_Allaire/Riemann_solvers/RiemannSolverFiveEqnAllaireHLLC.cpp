    ...
    const std::vector<boost::reference_wrapper<double> >& Q_R)
{
    ...
    if (d_dim == tbox::Dimension(1))
    {
        ...
        Z_L_ptr.reserve(d_num_species - 1);
        Z_R_ptr.reserve(d_num_species - 1);
        for (int si = 0; si < d_num_species - 1; si++)
        {
            Z_L_ptr.push_back(&(Q_L[d_num_species + d_dim.getValue() + 1 + si].get()));
            Z_R_ptr.push_back(&(Q_R[d_num_species + d_dim.getValue() + 1 + si].get()));
        }
    ...
    else if (d_dim == tbox::Dimension(2))
    {
        ...
        const double epsilon_B =
            ((Q_B[d_num_species + d_dim.getValue()].get()) -
                0.5*(Q_B[d_num_species].get())*(Q_B[d_num_species].get())/rho_B)/rho_B;
        
        const double epsilon_T =
            ((Q_T[d_num_species + d_dim.getValue()].get()) -
                0.5*(Q_T[d_num_species].get())*(Q_T[d_num_species].get())/rho_T)/rho_T;
        ...
        Z_B_ptr.reserve(d_num_species - 1);
        Z_T_ptr.reserve(d_num_species - 1);
        for (int si = 0; si < d_num_species - 1; si++)
        {
            Z_B_ptr.push_back(&(Q_B[d_num_species + d_dim.getValue() + 1 + si].get()));
            Z_T_ptr.push_back(&(Q_T[d_num_species + d_dim.getValue() + 1 + si].get()));
        }
    ...
    else if (d_dim == tbox::Dimension(3))
    {
        ...
        const double epsilon_F =
            ((Q_F[d_num_species + d_dim.getValue()].get()) -
                0.5*(Q_F[d_num_species].get())*(Q_F[d_num_species].get())/rho_F)/rho_F;
        ...
        Z_B_ptr.reserve(d_num_species - 1);
        Z_F_ptr.reserve(d_num_species - 1);
        for (int si = 0; si < d_num_species - 1; si++)
        {
            Z_B_ptr.push_back(&(Q_B[d_num_species + d_dim.getValue() + 1 + si].get()));
            Z_F_ptr.push_back(&(Q_F[d_num_species + d_dim.getValue() + 1 + si].get()));
        }
    ...
    const std::vector<boost::reference_wrapper<double> >& V_R)
{
    ...
    if (d_dim == tbox::Dimension(1))
    {
        ...
        for (int si = 0; si < d_num_species - 1; si++)
        {
            Z_L_ptr.push_back(&(V_L[d_num_species + d_dim.getValue() + 1 + si].get()));
            Z_R_ptr.push_back(&(V_R[d_num_species + d_dim.getValue() + 1 + si].get()));
        }
    ...
    else if (d_dim == tbox::Dimension(3))
    {
        ...
        if (s_star > 0)
        {
            ...
            Q_L[d_num_species + d_dim.getValue()] = rho_L*(V_L[d_num_species + 2].get());
            Q_L[d_num_species + d_dim.getValue()] = rho_L*(epsilon_L +
                0.5*((V_L[d_num_species].get())*(V_L[d_num_species].get()) +
                (V_L[d_num_species + 1].get())*(V_L[d_num_species + 1].get()) +
    ...
    const std::vector<boost::reference_wrapper<double> >& V_T)
{
    ...
    else if (d_dim == tbox::Dimension(2))
    {
        ...
        for (int si = 0; si < d_num_species - 1; si++)
        {
            Z_B_ptr.push_back(&(V_B[d_num_species + d_dim.getValue() + 1 + si].get()));
            Z_T_ptr.push_back(&(V_T[d_num_species + d_dim.getValue() + 1 + si].get()));
        }
    ...
    const std::vector<boost::reference_wrapper<double> >& V_F)
{
    ...
    else if (d_dim == tbox::Dimension(3))
    {
        ...
        {
            Z_B_ptr.push_back(&(V_B[d_num_species + d_dim.getValue() + 1 + si].get()));
            Z_F_ptr.push_back(&(V_F[d_num_species + d_dim.getValue() + 1 + si].get()));
        }
