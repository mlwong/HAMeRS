            ...
            equation_of_state_mixing_rules)
{
    ...
    {
        d_rho_Y_body = immersed_boundary_method_db->getRealVector("body_partial_densities");
    }
    else
    {
    ...
    if (static_cast<int>(d_rho_Y_body.size()) != d_num_species)
    {
        TBOX_ERROR(d_object_name
            << ": FlowModelImmersedBoundaryMethodFourEqnConservative::FlowModelImmersedBoundaryMethodFourEqnConservative()\n"
    ...
    {
        rho_body += d_rho_Y_body[si];
    }
    
    /*
     * Compute the mass fractions of the body.
     */
    
    std::vector<Real> Y_body;
    Y_body.reserve(d_num_species);
    for (int si = 0; si < d_num_species; si++)
    {
        Y_body.push_back(d_rho_Y_body[si]/rho_body);
    }
    
    /*
     * Get the pointers to the mass fractions of the body.
     */
    
    std::vector<const Real*> Y_body_ptr;
    Y_body_ptr.reserve(d_num_species);
    for (int si = 0; si < d_num_species; si++)
    {
        Y_body_ptr.push_back(&Y_body[si]);
    }
    ...
    const Real T_body = d_equation_of_state_mixing_rules->
        getTemperature(
            &rho_body,
            &d_p_body,
            Y_body_ptr);
    
    const Real epsilon_body = d_equation_of_state_mixing_rules->
 ...
 */
void FlowModelImmersedBoundaryMethodFourEqnConservative::setConservativeVariablesCellDataImmersedBoundaryGhosts(
    ...
    if (d_dim == tbox::Dimension(1))
    {
        ...
        for (int si = 0; si < d_num_species; si++)
        {
            ...
            for (int i = domain_lo_0; i < domain_lo_0 + domain_dim_0; i++)
            {
                ...
                {
                    // NEED TO BE CHANGED!!!
                    rho_Y[si][idx_cons_var] = d_rho_Y_body[si];
                }
