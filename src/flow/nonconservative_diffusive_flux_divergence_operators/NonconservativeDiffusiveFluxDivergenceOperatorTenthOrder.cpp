...
NonconservativeDiffusiveFluxDivergenceOperatorTenthOrder::NonconservativeDiffusiveFluxDivergenceOperatorTenthOrder(
 ...
 */
void
NonconservativeDiffusiveFluxDivergenceOperatorTenthOrder::printClassData(
    std::ostream& os) const
{
    os << "\nPrint NonconservativeDiffusiveFluxDivergenceOperatorTenthOrder object..."
       << std::endl;
    
    os << std::endl;
    
    os << "NonconservativeDiffusiveFluxDivergenceOperatorTenthOrder: this = "
       << (NonconservativeDiffusiveFluxDivergenceOperatorTenthOrder *)this
       << std::endl;
    os << "d_object_name = "
       << d_object_name
       << std::endl;
 ...
 */
void
NonconservativeDiffusiveFluxDivergenceOperatorTenthOrder::putToRestart(
   const HAMERS_SHARED_PTR<tbox::Database>& restart_db) const
{
    restart_db->putString("d_nonconservative_diffusive_flux_divergence_operator", "TENTH_ORDER");
}
    ...
    const hier::Patch& patch)
{
    ...
    const Real a_n =  Real(5)/Real(6);
    const Real b_n = -Real(5)/Real(21);
    const Real c_n =  Real(5)/Real(84);
    const Real d_n = -Real(5)/Real(504);
    const Real e_n =  Real(1)/Real(1260);
        ...
        for (int ei = 0; ei < d_num_eqn; ei++)
        {
            ...
            for (int vi = 0; vi < static_cast<int>(data_x[ei].size()); vi++)
            {
                    ...
                    == derivative_x_computed.end())
                {
                    ...
                    for (int j = -5; j < interior_dim_1 + 5; j++)
                    {
                        HAMERS_PRAGMA_SIMD
                        for (int i = 0; i < interior_dim_0; i++)
                        {
        ...
        for (int ei = 0; ei < d_num_eqn; ei++)
        {
            ...
            for (int vi = 0; vi < static_cast<int>(data_x[ei].size()); vi++)
            {
                    ...
                    == derivative_x_computed.end())
                {
                    ...
                    for (int k = -5; k < interior_dim_2 + 5; k++)
                    {
                        for (int j = -5; j < interior_dim_1 + 5; j++)
                        {
                            HAMERS_PRAGMA_SIMD
    ...
    const hier::Patch& patch)
{
    ...
    if (d_dim == tbox::Dimension(1))
    {
        TBOX_ERROR(d_object_name
            << ": NonconservativeDiffusiveFluxDivergenceOperatorTenthOrder::"
        ...
        for (int ei = 0; ei < d_num_eqn; ei++)
        {
            ...
            for (int vi = 0; vi < static_cast<int>(data_y[ei].size()); vi++)
            {
                    ...
                    == derivative_y_computed.end())
                {
                    ...
                    {
                        HAMERS_PRAGMA_SIMD
                        for (int i = -5; i < interior_dim_0 + 5; i++)
                        {
                            // Compute the linear indices.
                            const int idx = (i + num_diff_ghosts_0) +
                                (j + num_diff_ghosts_1)*diff_ghostcell_dim_0;
    ...
    const hier::Patch& patch)
{
    ...
    const Real a_n = -Real(5269)/Real(1800);
    const Real b_n =  Real(5)/Real(3);
    const Real c_n = -Real(5)/Real(21);
    const Real d_n =  Real(5)/Real(126);
    const Real e_n = -Real(5)/Real(1008);
    const Real f_n =  Real(1)/Real(3150);
