...
ConvectiveFluxReconstructorWCNS_HW6_LD_HLLC_HLL::ConvectiveFluxReconstructorWCNS_HW6_LD_HLLC_HLL(
    ...
    std::ostream& os) const
{
    ...
    os << "ConvectiveFluxReconstructorWCNS_HW6_LD_HLLC_HLL: this = "
       << (ConvectiveFluxReconstructorWCNS_HW6_LD_HLLC_HLL *)this
       << std::endl;
    os << "d_object_name = "
       << d_object_name
       << std::endl;
 ...
 */
void
ConvectiveFluxReconstructorWCNS_HW6_LD_HLLC_HLL::computeConvectiveFluxesAndSources(
    hier::Patch& patch,
    const double time,
    const double dt,
 ...
 */
void
ConvectiveFluxReconstructorWCNS_HW6_LD_HLLC_HLL::projectPrimitiveVariablesToCharacteristicFields(
 ...
 */
void
ConvectiveFluxReconstructorWCNS_HW6_LD_HLLC_HLL::projectCharacteristicVariablesToPhysicalFields(
 ...
 */
std::vector<double>
ConvectiveFluxReconstructorWCNS_HW6_LD_HLLC_HLL::computeSigma(
    const boost::multi_array<double, 2>& W_array)
{
    ...
    for (int ei = 0; ei < d_num_eqn; ei++)
    {
        const double r_1 = (fabs(2*(W_array[2][ei] - W_array[1][ei])*(W_array[1][ei] - W_array[0][ei])))/
            (pow((W_array[2][ei] - W_array[1][ei]), 2) + pow((W_array[1][ei] - W_array[0][ei]), 2) + EPSILON);
        
        const double r_2 = (fabs(2*(W_array[3][ei] - W_array[2][ei])*(W_array[2][ei] - W_array[1][ei])))/
            (pow((W_array[3][ei] - W_array[2][ei]), 2) + pow((W_array[2][ei] - W_array[1][ei]), 2) + EPSILON);
        
        const double r_3 = (fabs(2*(W_array[4][ei] - W_array[3][ei])*(W_array[3][ei] - W_array[2][ei])))/
            (pow((W_array[4][ei] - W_array[3][ei]), 2) + pow((W_array[3][ei] - W_array[2][ei]), 2) + EPSILON);
        
        const double r_4 = (fabs(2*(W_array[5][ei] - W_array[4][ei])*(W_array[4][ei] - W_array[3][ei])))/
            (pow((W_array[5][ei] - W_array[4][ei]), 2) + pow((W_array[4][ei] - W_array[3][ei]), 2) + EPSILON);
        
        sigma.push_back(fmin(fmin(r_1, r_2), fmin(r_3, r_4)));
    }
...
ConvectiveFluxReconstructorWCNS_HW6_LD_HLLC_HLL::computeTV(
    const boost::multi_array<double, 2>& W_array)
{
#ifdef DEBUG_CHECK_ASSERTIONS
    TBOX_ASSERT(static_cast<int>(W_array.shape()[0]) == 6);
    ...
    boost::multi_array<double, 2> TV(
        boost::extents[d_num_eqn][4]);
    
    for (int ei = 0; ei < d_num_eqn; ei++)
    {
        TV[ei][0] = fabs(W_array[0][ei] - W_array[1][ei]) + fabs(W_array[1][ei] - W_array[2][ei]);
        TV[ei][1] = fabs(W_array[1][ei] - W_array[2][ei]) + fabs(W_array[2][ei] - W_array[3][ei]);
        TV[ei][2] = fabs(W_array[2][ei] - W_array[3][ei]) + fabs(W_array[3][ei] - W_array[4][ei]);
        TV[ei][3] = fabs(W_array[3][ei] - W_array[4][ei]) + fabs(W_array[4][ei] - W_array[5][ei]);
    }
 ...
 */
boost::multi_array<double, 2>
ConvectiveFluxReconstructorWCNS_HW6_LD_HLLC_HLL::computeBeta(
    const boost::multi_array<double, 2>& W_array)
{
#ifdef DEBUG_CHECK_ASSERTIONS
    TBOX_ASSERT(static_cast<int>(W_array.shape()[0]) == 6);
 ...
 */
boost::multi_array<double, 2>
ConvectiveFluxReconstructorWCNS_HW6_LD_HLLC_HLL::computeBetaTilde(
    const boost::multi_array<double, 2>& W_array)
{
#ifdef DEBUG_CHECK_ASSERTIONS
    TBOX_ASSERT(static_cast<int>(W_array.shape()[0]) == 6);
 ...
 */
void
ConvectiveFluxReconstructorWCNS_HW6_LD_HLLC_HLL::performWENOInterpolation(
    std::vector<double>& W_L,
    std::vector<double>& W_R,
    const boost::multi_array<double, 2>& W_array)
{
    ...
    std::vector<double> sigma = computeSigma(W_array);
    ...
    boost::multi_array<double, 2> TV = computeTV(W_array);
    
    for (int ei = 0; ei < d_num_eqn; ei++)
    {
        double TV_ratio = fmax(fmax(fmax(TV[ei][0], TV[ei][1]), TV[ei][2]), TV[ei][3])/
            (fmin(fmin(fmin(TV[ei][0], TV[ei][1]), TV[ei][2]), TV[ei][3]) + EPSILON);
        ...
        double tau_6 = fabs(beta[ei][3] - 1.0/8*(beta[ei][0] + beta[ei][2] +
            6*beta[ei][1]));
        ...
        if ((TV_ratio > alpha_TV) &&
            (fmax(fmax(fmax(beta[ei][0], beta[ei][1]), beta[ei][2]), beta[ei][3])/
            (fmin(fmin(fmin(beta[ei][0], beta[ei][1]), beta[ei][2]), beta[ei][3]) + EPSILON)
             > alpha_beta))
        {
            // Compute linear weights d.
            d[0] = (2.0 - pow(sigma[ei], p))/32.0;
            d[1] = 5.0*(4.0 - pow(sigma[ei], p))/32.0;
            d[2] = 5.0*(2.0 + pow(sigma[ei], p))/32.0;
            d[3] = pow(sigma[ei], p)/32.0;
            
            for (int r = 0; r < 4; r++)
            {
                alpha.push_back(
                    d[r]*(EPSILON + pow(sigma[ei], p)*C + pow(tau_6/(beta[ei][r] + EPSILON), q)));
            }
        }
        else
        {
            ...
            {
                alpha.push_back(
                    d[r]*(C + pow(tau_6/(beta[ei][r] + EPSILON), q)));
            }
        }
        ...
        double tau_6_tilde = fabs(beta_tilde[ei][3] - 1.0/8*(beta_tilde[ei][0] +
        ...
        if ((TV_ratio > alpha_TV)  &&
            (fmax(fmax(fmax(beta_tilde[ei][0], beta_tilde[ei][1]), beta_tilde[ei][2]), beta_tilde[ei][3])/
            (fmin(fmin(fmin(beta_tilde[ei][0], beta_tilde[ei][1]), beta_tilde[ei][2]), beta_tilde[ei][3]) + EPSILON)
             > alpha_beta))
        {
            // Compute linear weights d.
            d[0] = (2.0 - pow(sigma[ei], p))/32.0;
            ...
            for (int r = 0; r < 4; r++)
            {
                alpha_tilde.push_back(
                    d[r]*(EPSILON + pow(sigma[ei], p)*C + pow(tau_6_tilde/(beta_tilde[ei][r] + EPSILON), q)));
            }
        }
        else
        {
            ...
            {
                alpha_tilde.push_back(
                    d[r]*(C + pow(tau_6_tilde/(beta_tilde[ei][r] + EPSILON), q)));
            }
        }
