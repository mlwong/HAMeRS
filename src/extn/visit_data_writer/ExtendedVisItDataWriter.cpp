   ...
   const int start_depth_index)
{
   ...
   if (!found_type) {
      boost::shared_ptr<SAMRAI::pdat::CellDataFactory<float> > ffactory(
         boost::dynamic_pointer_cast<SAMRAI::pdat::CellDataFactory<float>, SAMRAI::hier::PatchDataFactory>(
            factory));
      if (ffactory) {
         vdt = VISIT_FLOAT;
         vc = VISIT_CELL;
   ...
   if (!found_type) {
      boost::shared_ptr<SAMRAI::pdat::NodeDataFactory<float> > ffactory(
         boost::dynamic_pointer_cast<SAMRAI::pdat::NodeDataFactory<float>, SAMRAI::hier::PatchDataFactory>(
            factory));
      if (ffactory) {
         vdt = VISIT_FLOAT;
         vc = VISIT_NODE;
   ...
   if (!found_type) {
      boost::shared_ptr<SAMRAI::pdat::CellDataFactory<double> > dfactory(
         boost::dynamic_pointer_cast<SAMRAI::pdat::CellDataFactory<double>, SAMRAI::hier::PatchDataFactory>(
            factory));
      if (dfactory) {
         vdt = VISIT_DOUBLE;
         vc = VISIT_CELL;
   ...
   if (!found_type) {
      boost::shared_ptr<SAMRAI::pdat::NodeDataFactory<double> > dfactory(
         boost::dynamic_pointer_cast<SAMRAI::pdat::NodeDataFactory<double>, SAMRAI::hier::PatchDataFactory>(
            factory));
      if (dfactory) {
         vdt = VISIT_DOUBLE;
         vc = VISIT_NODE;
   ...
   if (!found_type) {
      boost::shared_ptr<SAMRAI::pdat::CellDataFactory<int> > ifactory(
         boost::dynamic_pointer_cast<SAMRAI::pdat::CellDataFactory<int>, SAMRAI::hier::PatchDataFactory>(
            factory));
      if (ifactory) {
         vdt = VISIT_INT;
         vc = VISIT_CELL;
   ...
   if (!found_type) {
      boost::shared_ptr<SAMRAI::pdat::NodeDataFactory<int> > ifactory(
         boost::dynamic_pointer_cast<SAMRAI::pdat::NodeDataFactory<int>, SAMRAI::hier::PatchDataFactory>(
            factory));
      if (ifactory) {
         vdt = VISIT_INT;
         vc = VISIT_NODE;
   ...
   double simulation_time)
{
   ...
   for (int ln = 0; ln < hierarchy->getNumberOfLevels(); ++ln) {
      ...
      if (!d_is_multiblock && *sorted_box_level != unsorted_box_level) {
         TBOX_ERROR(
            "ExtendedVisItDataWriter: Encountered existing limitation of ExtendedVisItDataWriter\n"
...
//#define USE_BATON_BARRIERS
...
#endif
   {
      ...
      sprintf(temp_buf, "/processor_cluster.%05d.samrai", d_my_file_cluster_number);
         ...
         for (int depth_id = 0; depth_id < ipi->d_depth; ++depth_id) {
            ...
            if ((ipi->d_is_material_state_variable)) {
               ...
               if ((return_code != SAMRAI::appu::VisMaterialsDataStrategy::VISIT_MIXED)
                   && (return_code !=
                       SAMRAI::appu::VisMaterialsDataStrategy::VISIT_ALLONE)
                   && (return_code !=
                       SAMRAI::appu::VisMaterialsDataStrategy::VISIT_ALLZERO)) {
                  TBOX_ERROR(
                     "ExtendedVisItDataWriter::packMaterialsData()"
 ...
 */
      ...
      for (ln = coarsest_plot_level; ln <= finest_plot_level; ++ln) {
         ...
         pn = 0;
         for (SAMRAI::hier::BoxContainer::const_iterator itr = boxes.begin();
   ...
   const boost::shared_ptr<SAMRAI::tbox::Database>& basic_HDFGroup)
{
   ...
   for (ln = 0; ln <= finest_level; ++ln) {
      ...
      if (ln != finest_level) {
         ...
         const SAMRAI::hier::BoxContainer& global_child_boxes =
      ...
      for (SAMRAI::hier::RealBoxConstIterator bi(coarser_boxes.realBegin());
   ...
   const variable_centering centering)
{
   ...
   if (d_dim < SAMRAI::tbox::Dimension(2) || d_dim > SAMRAI::tbox::Dimension(3)) {
      TBOX_ERROR(
         d_object_name << "ExtendedVisItDataWriter::packPatchDataIntoDoubleBuffer()"
   ...
   switch (data_type) {
      ...
      case VISIT_FLOAT: {
         ...
         if (centering == VISIT_CELL) {
            ...
            SAMRAI::pdat::CellData<float> cell_copy(plot_box,
                                            1,
                                            SAMRAI::hier::IntVector::getZero(d_dim));
            ...
            SAMRAI::pdat::NodeData<float> node_copy(plot_box,
                                            1,
                                            SAMRAI::hier::IntVector::getZero(d_dim));
      ...
      case VISIT_DOUBLE: {
         ...
         if (centering == VISIT_CELL) {
            ...
            SAMRAI::pdat::CellData<double> cell_copy(plot_box,
                                             1,
                                             SAMRAI::hier::IntVector::getZero(d_dim));
            pdata->copy2(cell_copy);
            dat_ptr = cell_copy.getPointer();
            ...
            SAMRAI::pdat::NodeData<double> node_copy(plot_box,
                                             1,
                                             SAMRAI::hier::IntVector::getZero(d_dim));
            pdata->copy2(node_copy);
            dat_ptr = node_copy.getPointer();
