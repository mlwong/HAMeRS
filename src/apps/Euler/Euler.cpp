        ...
        d_Euler_boundary_conditions_db_is_from_restart(false)
{
    ...
    d_filename_output_statistics = "statistics.txt";
    
    const tbox::SAMRAI_MPI& mpi(tbox::SAMRAI_MPI::getSAMRAIWorld());
    if (mpi.getRank() == 0)
    {
        std::ofstream f_out;
        f_out.open(d_filename_output_statistics.c_str(), std::ios::app);
        if (!f_out.is_open())
        {
        ...
        f_out << "TIME             " << "\t" << "MIXING_WIDTH_X" << std::endl;
        f_out.close();
    }
}
        ...
        for (int ei = 0; ei < d_flow_model->getNumberOfEquations(); ei++)
        {
            ...
            for (int j = 0; j < interior_dim_1; j++)
            {
                ...
                for (int i = 0; i < interior_dim_0; i++)
                {
                    ...
                    const int idx_flux_x_L = i + j*(interior_dim_0 + 1);
                    const int idx_flux_x_R = (i + 1) + j*(interior_dim_0 + 1);
            ...
            for (int j = 0; j < interior_dim_1; j++)
            {
                ...
                for (int i = 0; i < interior_dim_0; i++)
                {
                    ...
                    const int idx_flux_y_B = j + i*(interior_dim_1 + 1);
                    const int idx_flux_y_T = (j + 1) + i*(interior_dim_1 + 1);
                    
                    Q[ei][idx] -= ((F_y[idx_flux_y_T] - F_y[idx_flux_y_B])/dx_1);
                }
            }
            ...
            for (int k = 0; k < interior_dim_2; k++)
            {
                for (int j = 0; j < interior_dim_1; j++)
                {
                    ...
                    for (int i = 0; i < interior_dim_0; i++)
                    {
                        ...
                        const int idx_flux_y_B = j +
                            k*(interior_dim_1 + 1) +
                            i*(interior_dim_1 + 1)*interior_dim_2;
                        
                        const int idx_flux_y_T = (j + 1) +
                            k*(interior_dim_1 + 1) +
                            i*(interior_dim_1 + 1)*interior_dim_2;
            ...
            for (int k = 0; k < interior_dim_2; k++)
            {
                for (int j = 0; j < interior_dim_1; j++)
                {
                    ...
                    for (int i = 0; i < interior_dim_0; i++)
                    {
                        ...
                        const int idx_flux_z_B = k +
                            i*(interior_dim_2 + 1) +
                            j*(interior_dim_2 + 1)*interior_dim_0;
                        
                        const int idx_flux_z_F = (k + 1) +
                            i*(interior_dim_2 + 1) +
                            j*(interior_dim_2 + 1)*interior_dim_0;
                        
                        Q[ei][idx] -= ((F_z[idx_flux_z_F] - F_z[idx_flux_z_B])/dx_2);
                    }
                }
            }
    ...
    const double output_time)
{
    ...
    std::string statistics_variable_key = "MIXING_WIDTH_X";
    ...
    TBOX_ASSERT(physical_domain.size() == 1);
    ...
    {
        if (statistics_variable_key == "MIXING_WIDTH_X")
        {
            TBOX_ASSERT(d_num_species == 2);
            ...
            {
                const int finest_level_dims_0 = finest_level_dims[0];
                
                const double L_x = x_hi[0] - x_lo[0];
                const double L_y = x_hi[1] - x_lo[1];
                const double dx_finest = L_x/finest_level_dims_0;
                
                double* Y_avg_local = (double*)std::malloc(finest_level_dims_0*sizeof(double));
                double* Y_avg_global = (double*)std::malloc(finest_level_dims_0*sizeof(double));
                
                for (int i = 0; i < finest_level_dims_0; i++)
                {
                    Y_avg_local[i] = 0.0;
                    Y_avg_global[i] = 0.0;
                }
