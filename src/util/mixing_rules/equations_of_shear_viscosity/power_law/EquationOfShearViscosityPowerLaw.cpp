 ...
 */
void
EquationOfShearViscosityPowerLaw::printClassData(
    std::ostream& os) const
{
    os << "\nPrint EquationOfShearViscosityPowerLaw object..."
       << std::endl;
       
    os << std::endl;
    
    os << "EquationOfShearViscosityPowerLaw: this = "
       << (EquationOfShearViscosityPowerLaw *)this
       << std::endl;
 ...
 */
Real
EquationOfShearViscosityPowerLaw::getShearViscosity(
    ...
    const std::vector<const Real*>& molecular_properties) const
{
    ...
    const Real& mu_ref = *(molecular_properties[0]);
    const Real& T_ref  = *(molecular_properties[1]);
    const Real& power  = *(molecular_properties[2]);
    
    const Real& T = *temperature;
    
    const Real mu = mu_ref*std::pow(T/T_ref, power);
    return mu;
}
 ...
 */
void
EquationOfShearViscosityPowerLaw::computeShearViscosity(
    ...
    const hier::Box& domain) const
{
    ...
    if (d_dim == tbox::Dimension(1))
    {
        ...
        for (int i = domain_lo_0; i < domain_lo_0 + domain_dim_0; i++)
        {
            ...
            mu[idx_shear_viscosity] = mu_ref*std::pow(T[idx_temperature]/T_ref, power);
        }
    }
    ...
    const hier::Box& domain) const
{
    ...
    Real* mu_ref = data_molecular_properties->getPointer(0);
    Real* T_ref  = data_molecular_properties->getPointer(1);
    Real* power  = data_molecular_properties->getPointer(2);
