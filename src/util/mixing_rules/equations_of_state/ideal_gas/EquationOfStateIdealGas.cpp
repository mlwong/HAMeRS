    ...
    const hier::Box& domain) const
{
    ...
    if (domain.empty())
    {
        data_isochoric_specific_heat_capacity->fillAll(c_v);
    }
    else
    {
#ifdef HAMERS_DEBUG_CHECK_ASSERTIONS
    ...
    const hier::Box& domain) const
{
    ...
    const double& c_p = *(thermo_properties[2]);
    
    if (domain.empty())
    {
        data_isobaric_specific_heat_capacity->fillAll(c_p);
    }
    else
    {
#ifdef HAMERS_DEBUG_CHECK_ASSERTIONS
    ...
    const hier::Box& domain) const
{
    ...
    if (domain.empty())
    {
        data_partial_internal_energy_partial_pressure->fillAll(xi);
    }
    else
    {
#ifdef HAMERS_DEBUG_CHECK_ASSERTIONS
