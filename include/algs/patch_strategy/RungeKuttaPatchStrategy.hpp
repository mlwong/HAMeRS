...
/**
 * Class RungeKuttaPatchStrategy is an abstract base class defining the
 * interface between an RungeKuttaLevelIntegrator object and operations
 * applied to a single patch in a structured AMR hierarchy.   The operations
 * include patch initialization, dt calculation, hyperbolic flux computation,
 ...
 * and the RungeKuttaLevelIntegrator to work together, the concrete
   ...
   public xfer::CoarsenPatchStrategy
{
    public:
         ...
         * routine as well, since the Runge-Kutta level integrator provides the
         ...
         * registerModelVariables() function is called.  The Runge-Kutta level
        ...
        /**
         * Compute TIME INTEGRALS of hyperbolic fluxes to be used in finite
         * difference for patch integration. That is, it is assumed that this
         * numerical routine will compute the fluxes corresponding to the cell
         ...
         * and fluxes computed in computeHyperbolicFluxesAndSourcesOnPatch() routine.
         * Note that the computeHyperbolicFluxesAndSourcesOnPatch() routine computes
         * TIME INTEGRALs of the numerical fluxes and sources (e.g., they have been
         * multiplied by dt) so the time-stepping routine should be consistent with
         * that.
         *
         * @param patch                 patch that RK step is being applied
         * @param time                  simulation time when the routine is called
         * @param dt                    timestep
         * @param alpha                 vector of coefficients applied to the intermediate solution in the RK step
        ...
        /**
         * Correct the hyperbolic flux at the coarse-fine boundaries during a flux
         * synchronization step. Note that the computeHyperbolicFluxesAndSourcesOnPatch()
         * routine computes TIME INTEGRALs of the numerical fluxes and sources (e.g., they
         * have been multiplied by dt) so the hyperbolic flux synchronization routine should
         ...
         * and before computeHyperbolicFluxesAndSourcesOnPatch(). The arguments are:
        ...
        /**
         * The Runge-Kutta integrator controls the context for the data to be used
