...
/**
 * Class RungeKuttaLevelIntegrator provides routines needed to
 * integrate a system of conservation laws on a structured
 ...
 * base class RungeKuttaPatchStrategy.
 *
    ...
    public tbox::Serializable
{
    public:
         ...
         * - \b HYP_FLUX       {Face-centered double values used in conservative
         *                      difference and synchronization (i.e., refluxing)
         *                      process.  A corresponding variable to store hyperbolic
         *                      flux integral information is created for each
         *                      HYP_FLUX variable.}
         * - \b SOURCE         {Cell-centered double values that do not need
         *                      synchronization.}
        ...
        /**
         * The destructor for RungeKuttaLevelIntegrator unregisters
         ...
         * time (current_time) to new time (new_time) using explicit Runge-Kutta
         * methods. This routine is used to advance the solution on each level in
         * the hierarchy and during time-dependent regridding procedures, such as
         * Richardson extrapolation. The boolean arguments are used to determine
         * the state of the algorithm and the data when the advance routine is
         * called. The first_step and last_step indicate whether the step is the
         * first or last in the current timestep sequence on the level. Typically,
         * the current timestep sequence means each step on the level between
         * advance steps on a coarser level in the hierarchy, if one exists. The
         * regrid_advance value is true when the advance is called as part of a
         * time-dependent regridding procedure.  Usually when this happens, the
         * results of the solution advance will be discarded.  So, for example,
         * when this is true hyperbolic flux information is not maintained and
         * hyperbolic flux integrals are not updated. The final boolean argument
         * indicates whether or not the level resides in the hierarchy.  For example,
         * during time-dependent regridding, such as Richardson extrapolation, a
         * temporary level that is not in the hierarchy is created and advanced.
         * Then, a communication schedule must be generated for the level before
         * the advance begins.
         *
         ...
         * according to the standard AMR hyperbolic flux correction algorithm.
        ...
        /**
         * Register a variable with the Runge-Kutta integration algorithm.  The
         ...
         * Runge-Kutta patch model when the variable registration process is
         ...
         * in the concrete RungeKuttaPatchStrategy object manipulate;
    ...
    protected:
        ...
        /*
         * Pre-process hyperbolic flux and source storage before advancing solution
         * on level from cur_time to new_time.  The boolean flags are used to 
         * determine how hyperbolic flux and hyperbolic flux integral storage
         * is allocated and initialized. These are needed since the advanceLevel()
         * routine is used for both level integration and time-dependent error
         * estimation.
         */
        virtual void
        ...
        /*
         * Post-process hyperbolic flux and source storage after advancing solution on
         * level. The boolean flag is used to determine how hyperbolic flux and
         * hypberolic flux integral storage is copied and de-allocated. This is needed 
         * since the advanceLevel() routine is used for both level integration and
        ...
        /**
         * Apply the standard AMR Runge-Kutta hyperbolic flux synchronization process
         * preserve conservation properties in the solution between the fine level
         * and the coarse level.  The sync_time argument indicates the time at which
         * the solution on the two levels is being synchronized.  The variable
         * coarse_sim_time indicates the previous simulation time on the coarse
         * level (recall the conservative difference will be repeated on the level
         * during the synchronization process).  After the synchronization, the
         * hyperbolic flux and hyperbolic flux integral data storage is reset on
         * the levels.
         *
         * @pre fine_level
         * @pre coarse_level
